#![no_std]
use soroban_sdk::{contract, contracttype, contractimpl, Address, BytesN, Env, Vec, Symbol, token};
...
pub struct CircleInfo {
    ...
    pub max_members: u32, // Track with Soroban-supported integer width
...
pub struct TaxReport {
    pub circle_id: u64,
    pub user: Address,
    pub gross_interest_total_circle: u64,
...
impl SoroSusuTrait for SoroSusu {
    ...
    fn get_tax_report(env: Env, user: Address, circle_id: u64) -> TaxReport {
        ...
        TaxReport {
            circle_id,
            user: user.clone(),
            gross_interest_total_circle: env.storage().instance().get(&DataKey::GrossInterestTotal(circle_id)).unwrap_or(0),
